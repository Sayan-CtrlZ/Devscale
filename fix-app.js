...
// Find the boundaries of the <main> block
...
if (startIndex !== -1 && endIndex !== -1) {
  // Remove the <main> block and replace it with Routes
...
// Rename Link from lucide to LinkIcon to avoid conflict with react-router-dom Link
data = data.replace('Link, Lock', 'Link as LinkIcon, Lock');

// Add Route imports if completely missing
...
function ScrollToTop() {
  ...
  // Wrap with <Router> and <ScrollToTop/>
  data = data.replace(
...
// Convert Desktop navigation <a> -> <Link>
...
// Convert Start Project button -> Link
data = data.replace(
...
// Convert Mobile map navigation -> static Links
